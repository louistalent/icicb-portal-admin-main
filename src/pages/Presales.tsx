...
const Presales = () => {
	const { user, presale, update, updated, getError } = useStore()
	const PREFIX = 'icvip'
	...
	const getTotal = async () => {
		const response = await request("/get-presale-total", {}, { 'x-admin-token': user?.token })
		if (response !== null && response.result) {
			update({ presale:response.result, updated:now() })
		}
	}
	...
	const handleFiles = async (text : string) => {
		if (typeof text==='string') {
			...
			if (Object.keys(dups).length) {
				update({loading : true});
				const result = await request("/import-presale-code",  {data:Object.keys(dups)}, { 'x-admin-token': user?.token });
	...
	/* const handleFiles = async (text : string) => {
		...
		if (typeof text==='string') {
			...
			if (dups.length) {
				const result = await call("/import-presale-code",  {data:dups});
				if (result !== null) {
					if(result.error !== undefined) {  
	...
	const generate = async(count: number) => {
		...
		try {
			...
			for ( let i=0;i< count; i++ ) {
				const code = generateCode("icvip")
				if (isValidCode("icvip", code)) console.log('valid code ' + code)
				ws.push(code)
				setAddrs(ws)
				setProgress(i / count)
	...
	const download = () => {
		...
		link.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(addrs.join('\r\n')))
  		link.setAttribute('download', `presale-code-${new Date().getTime()}.csv`);
			...
			<div className="row">
				<div className="col-4 col-md-12">
					<div className="col-12">
						...
						/>
						<UploadDialog type="presale" cbData={handleFiles} />
					</div>
				</div>
